        ...
        <form onSubmit={onSubmit} className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <input className="rounded-xl border border-slate-300 px-4 py-2" placeholder="Stock" type="number" value={form.stockQuantity} onChange={(e) => setForm({ ...form, stockQuantity: e.target.value })} />
            </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {products.map((p) => (
              <article key={p.id} className="rounded-xl border border-slate-200 p-3">
                <img src={p.imageUrls?.[0]} alt={p.name} className="h-28 w-full rounded-lg object-cover" />
